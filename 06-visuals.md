...
**Signature:**
```go
proton.Rect(win *Win, c color.NRGBA, widthDp, heightDp float32)
```
...
**Signature:**
```go
proton.RoundRect(win *Win, c color.NRGBA, widthDp, heightDp, radiusDp float32)
```
...
```go
proton.Card(win, proton.RGB(0x2a2a3e), 12, 16, func(win *proton.Win) {
...
**Signature:**
```go
proton.Card(win *Win, bg color.NRGBA, cornerDp, padDp float32, content func(*Win))
```
...
proton.List(win, &scroll, len(projects), func(win *proton.Win, i int) {
    p := projects[i]
    proton.PadV(win, 4, func(win *proton.Win) {
        proton.Card(win, proton.RGB(0x1e1e2e), 8, 12, func(win *proton.Win) {
...
**Signature:**
```go
proton.Badge(win *Win, bg color.NRGBA, fg color.NRGBA, text string)
```
...
proton.Row(win,
    ...
    func(win *proton.Win) { proton.Badge(win, proton.RGB(0xa3be8c), proton.RGB(0x2e3440), "GUI") },
    func(win *proton.Win) { proton.Gap(win, 4) },
    func(win *proton.Win) { proton.Badge(win, proton.RGB(0xebcb8b), proton.RGB(0x2e3440), "v1.0") },
)
```
...
proton.Image(win *Win, img proton.ImageOp, widthDp, heightDp float32)
```
...
proton.MinSize(win, 120, 40, func(win *proton.Win) {
...
proton.MaxWidth(win, 400, func(win *proton.Win) {
...
**Signatures:**
```go
proton.MinSize(win *Win, widthDp, heightDp float32, fn func(*Win))
proton.MaxWidth(win *Win, widthDp float32, fn func(*Win))
```
