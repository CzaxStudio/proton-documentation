...
**Signature:**
```go
proton.Button(win *Win, state *proton.Clickable, label string) bool
```
...
**Signature:**
```go
proton.OutlineButton(win *Win, state *proton.Clickable, label string) bool
```
...
```go
// icon comes from gioui.org/widget, loaded with widget.NewIcon()
...
**Signature:**
```go
proton.IconButton(win *Win, state *proton.Clickable, icon *widget.Icon, desc string) bool
```
...
if proton.Tappable(win, &rowClick, func(win *proton.Win) {
    // draw whatever you want in here
    proton.Card(win, proton.RGB(0x2a2a2a), 8, 12, func(win *proton.Win) {
...
**Signature:**
```go
proton.Tappable(win *Win, state *proton.Clickable, content func(*Win)) bool
```
...
proton.List(win, &scroll, len(items), func(win *proton.Win, i int) {
    if proton.Tappable(win, &u.rows[i], func(win *proton.Win) {
        proton.PadV(win, 10, func(win *proton.Win) {
